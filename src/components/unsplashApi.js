...
export default function Unsplash(imageType, imageTerms, imageOrderBy, imageOrientation, imageColor, imagePage, imagePerPage) {
  ...
  useEffect(
    () => {
      ...
      const today = GetUtils.GetFormatDateTime(GetUtils.GetByDay(), 'yyyy_MM_dd')
      ...
      const getUnsplashTopics = async () => {
        ...
        await axios
          ...
          .then(response => {
            ...
            setIsLoading(false)
            GetUtils.SetLocalStore(storeKey, extractResponseData)
          })
      ...
      if (storeKey in localStorage) {
        setUnsplashImages(GetUtils.GetLocalStore(storeKey))
        setIsLoading(false)
      } else {
        if (imageType === 'topics') {
          getUnsplashTopics()
